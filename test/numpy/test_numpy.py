...
def test_allow_threads(np, consumer, replies):
    allow_threads_pointer = np.ALLOW_THREADS
    allow_threads_pointer.get()
    consumer.listen()
...
def test_delete_array(np, consumer, replies):
    ...
    split_array_ptr.get()
    split_array_1_ptr.get()
    new_deleted_result_ptr.get()
    consumer.listen()
    ptr1_result = replies[split_array_ptr.id]
...
def test_array_unique(np, consumer, replies):
    ...
    unique_ptr.get()
    unique_index_ptr.get()
    unique_inverse_ptr.get()
    unique_counts_ptr.get()
    consumer.listen()
