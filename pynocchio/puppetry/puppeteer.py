...
class Puppeteer:
    ...
    def _mirror_class(self, lib, parent_path: str = ""):
        """'This method creates a placeholder for every member of this module."""
    ...
    @staticmethod
    def placeholder_class(cls, parent_path, broker, processed_classes) -> ClassPointer:
        """attrs = {}
        for name, member in inspect.getmembers(cls):
            ...
            if inspect.isfunction(member) or inspect.ismethod(member) or inspect.isbuiltin(member):
                attrs[name] = Puppeteer.inject_callable_member(member, current_path, broker)
            elif inspect.isclass(member) and member not in processed_classes:
                processed_classes.add(member)
                dummy_nested_class = Puppeteer.placeholder_class(member, current_path, broker, processed_classes)
                attrs[name] = dummy_nested_class
            elif not name.startswith("__") and not callable(member):
                if callable(member):
                    attrs[name] = Puppeteer.inject_callable_member(member, current_path, broker)
                else:
                    attrs[name] = Puppeteer.placeholder_variable(current_path,broker)
        DummyClass = type(cls.__name__, (ClassPointer,), attrs)
        """
    ...
    def inject_callable_member(member, path, broker) -> Callable:
        ...
        if inspect.isfunction(member):
            ...
            def placeholder_function(*args, **kwargs):
                function_pointer = FunctionPointer(path=path, args=args, kwargs=kwargs)
            ...
            def placeholder_method(*args, **kwargs):
                method_pointer = MethodPointer(path=path, args=args, kwargs=kwargs)
            ...
            def placeholder_builtin(*args, **kwargs):
                builtin_pointer = BuiltinPointer(path=path, args=args, kwargs=kwargs)
    ...
    def placeholder_module(member, path, modules, broker) -> Puppeteer:
        return Puppeteer(lib=member, modules=modules, parent_path=path, broker=broker)
